...
<article class="guild-directory-card" data-guild-card>
    <div class="guild-directory-card-header">
        <div class="guild-directory-logo {{ $imageUrl ? 'has-image' : 'has-fallback' }}" data-guild-logo-wrap>
            @if($imageUrl)<img src="{{ $imageUrl }}" alt="{{ $union->display_title }}" loading="lazy" decoding="async" @if(image_srcset($union->primary_image)) srcset="{{ image_srcset($union->primary_image) }}" sizes="(max-width: 768px) 100vw, 400px" @endif data-guild-logo>@endif
