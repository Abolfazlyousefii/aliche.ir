                ...
                <div class="tab-panel {{ $loop->first ? 'active' : '' }}" data-tab-panel="{{ $panel }}">
                    <div class="representative-layout">
                        <div class="representative-map">
                            ...
                            <img alt="{{ $data['label'] }}" class="map-img" src="{{ asset('assets/img/asnaf-gorgan-default.jpg') }}" loading="lazy" decoding="async">
                        </div>
                        <aside class="people-panel" data-search-area>
                            ...
                            <div class="people-scroll-wrap"><ul class="person-list" data-union-results>
                                @forelse(($data['items'] ?? collect()) as $union)
                                    <li><a href="{{ route('guilds.show', $union->slug) }}" class="d-flex align-items-center gap-2 text-decoration-none"><span class="person-avatar avatar-{{ ($loop->iteration % 6) + 1 }}">@if($union->primary_image)<img src="{{ $union->primary_image_url }}" alt="{{ $union->display_title }}" loading="lazy" decoding="async">@endif</span><div><strong>{{ $union->display_title }}</strong><small>{{ plain_text($union->short_description ?: $union->manager_name ?: $union->union_type_label, 90) }}</small></div></a></li>
                                @empty
                                    <li><span class="person-avatar avatar-1"></span><div><strong>اتحادیه‌ای ثبت نشده است</strong><small>به‌زودی اطلاعات اتحادیه‌ها نمایش داده می‌شود.</small></div></li>
                                @endforelse
                            </ul></div>
                        </aside>
                    </div>
