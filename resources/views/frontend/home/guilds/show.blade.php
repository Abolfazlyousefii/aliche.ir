...
@php
    ...
    $showContact = $union->isSectionEnabled('show_contact', true);
    ...
    $navItems = collect([
        ...
        ['visible' => $contentLinks->isNotEmpty(), 'id' => 'guild-services', 'label' => 'خدمات و محتوا'],
    ...
    $quickActions = collect([
        ['visible' => $showManager, 'id' => 'guild-manager', 'title' => 'رئیس اتحادیه', 'icon' => 'manager'],
        ['visible' => $showMembers, 'id' => 'guild-board', 'title' => 'هیئت‌مدیره', 'icon' => 'members'],
        ['visible' => $showCommissions, 'id' => 'guild-commissions', 'title' => 'کمیسیون‌ها', 'icon' => 'commission'],
        ['visible' => $showNews, 'id' => 'guild-news', 'title' => 'اخبار', 'icon' => 'news'],
        ['visible' => $showContact, 'id' => 'guild-contact', 'title' => 'تماس', 'icon' => 'contact'],
    ])->filter(fn ($item) => $item['visible'])->values();
...
<main class="guild-profile-page" data-guild-profile>
    <section class="guild-profile-hero" @if($union->cover_image) style="--guild-profile-cover:url('{{ $assetImage($union->cover_image, '') }}')" @endif>
                ...
                <section class="guild-profile-section" id="guild-manager" data-guild-profile-section>
                    ...
                    <div class="guild-profile-manager">
                        ...
                        <div class="guild-profile-manager__info">
                            ...
                            <strong>رئیس {{ $union->display_title }}</strong>
                            <p>اطلاعات تکمیلی و شرح سوابق رئیس اتحادیه پس از ثبت توسط مدیریت اتحادیه در این بخش نمایش داده می‌شود.</p>
                            @if($presidentButtons->isNotEmpty() || filled($unionPhoneHref) || filled($union->email))
                                <div class="guild-profile-manager__actions">
                                    @forelse($presidentButtons as $button)
                                        <a href="{{ $button['url'] }}" target="{{ $button['target'] ?? '_self' }}" @if(($button['target'] ?? '_self') === '_blank') rel="noopener noreferrer" @endif>{{ $button['title'] }}</a>
                                    @empty
                                        @if(filled($unionPhoneHref))<a href="tel:{{ $unionPhoneHref }}">تماس با اتحادیه</a>@endif
                    ...
                    <div class="guild-profile-commissions">
                        @foreach($union->commissions as $commission)
                            <article class="guild-profile-commission">
                                <span class="guild-profile-commission__number">{{ fa_number($loop->iteration) }}</span>
                    ...
                    <div class="guild-profile-documents">
                        @foreach($union->rules as $rule)
                            <article class="guild-profile-document">
                                <span><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M6 3h9l4 4v14H6zM15 3v5h5M9 13h7M9 17h5"/></svg></span>
                ...
                <section class="guild-profile-section" id="guild-education" data-guild-profile-section>
                    ...
                    <div class="guild-profile-education">
                        @foreach($union->educations as $education)
                            ...
                            <a href="{{ $educationUrl }}" @if(str_starts_with($educationUrl, 'http')) target="_blank" rel="noopener noreferrer" @endif><span><svg aria-hidden="true" viewBox="0 0 24 24"><path d="m3 9 9-5 9 5-9 5-9-5Z"/><path d="M6 11v5c3 2 9 2 12 0v-5"/></svg></span><div><h3>{{ $education->title }}</h3>@if($education->description)<p>{{ $plain($education->description, 150) }}</p>@endif</div></a>
                        @endforeach
                    </div>
                </section>
            @endif
                    ...
                    @if($hasContactData)
                        ...
                        @if($union->isSectionEnabled('show_social_links', true) && $socialLinks->isNotEmpty())
                            <div class="guild-profile-socials">
                                @foreach($socialLinks as $name => $url)<a href="{{ $url }}" target="_blank" rel="noopener noreferrer"><span>{{ ['instagram'=>'اینستاگرام','telegram'=>'تلگرام','whatsapp'=>'واتساپ','eitaa'=>'ایتا','bale'=>'بله','rubika'=>'روبیکا','website'=>'وب‌سایت'][$name] ?? $name }}</span><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M8 12a4 4 0 0 0 4 4h4a4 4 0 0 0 0-8h-2M16 12a4 4 0 0 0-4-4H8a4 4 0 0 0 0 8h2"/></svg></a>@endforeach
                            </div>
                        @endif
                    @else
                        <div class="guild-profile-empty"><svg aria-hidden="true" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="M12 8v5M12 17h.01"/></svg><p>شماره تماس، نشانی و ساعات کاری این اتحادیه هنوز ثبت نشده است. پس از تکمیل اطلاعات، راه‌های ارتباطی در همین بخش فعال می‌شوند.</p></div>
                    @endif
                </section>
            @endif
        </div>
